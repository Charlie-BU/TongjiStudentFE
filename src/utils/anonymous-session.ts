...
export const removeAnonymousSession = (sessionId: string): void => {
    ...
    // 按 lastActiveAt 排序
    filteredSessions.sort((a, b) => new Date(b.lastActiveAt).getTime() - new Date(a.lastActiveAt).getTime());
    localStorage.setItem(ANONYMOUS_SESSIONS_KEY, JSON.stringify(filteredSessions));
};
