...
export function ChatArea() {
  ...
  async function submitQuestion(value = input): Promise<void> {
    ...
    try {
      for await (const event of streamMockReply(question, controller.signal)) {
        setTurns((currentTurns) =>
      ...
      if (isAbortError(error)) {
        setTurns((currentTurns) =>
          currentTurns.map((turn) =>
            turn.id === turnId ? { ...turn, state: "aborted" } : turn,
          ),
        );
      }
    } finally {
  ...
  return (
    <main className="chat-shell">
      <header className="chat-header">
        <div>
          <Text className="brand-kicker">TongjiStudent</Text>
          <Title level={3}>同济同学</Title>
        </div>
        <Tag color="blue">Mock 流式演示</Tag>
      </header>

      <section className="chat-main" aria-label="同济同学对话">
        {turns.length === 0 ? (
          <div className="welcome-panel">
            <Tag color="processing">校园信息助手</Tag>
            <Title>有什么校园问题想问？</Title>
            <Paragraph type="secondary">
              这里使用 Mock 事件模拟 Agent 的检索过程与逐字回答。
            </Paragraph>
            ...
            {turns.map((turn) => (
              <article key={turn.id} className="chat-turn">
                ...
                <div className="assistant-section">
                  <AgentActivity key={`${turn.id}-${turn.state}`} turn={turn} />
                  <div className="message assistant-message">
                    {turn.answer ? (
                      <Paragraph className="answer-text">{turn.answer}</Paragraph>
                    ) : turn.state === "streaming" ? (
                      <Text type="secondary">正在准备回答…</Text>
                    ) : (
                      <Text type="secondary">本轮回答已停止。</Text>
                    )}
                  </div>
                </div>
              </article>
    ...
    <div className="activity-content">
      ...
      {turn.activities.map((activity) => (
        <div key={activity.id} className="activity-row">
          <span className={`activity-dot ${activity.state}`} aria-hidden="true" />
          <div>
            <Text>{activity.label}</Text>
            {activity.detail ? <Text type="secondary">{activity.detail}</Text> : null}
            {activity.durationMs ? (
              <Text type="secondary">耗时 {activity.durationMs}ms</Text>
            ) : null}
          </div>
        </div>
      ))}
...
function updateTurn(turn: ChatTurn, turnId: string, event: StreamEvent): ChatTurn {
  if (turn.id !== turnId) {
    return turn;
  }
  switch (event.type) {
    ...
    case "tool_started":
      return {
        ...
        activities: [
          ...
          { id: event.id, label: event.label, state: "running" as const },
        ],
      };
    case "tool_completed":
      return {
