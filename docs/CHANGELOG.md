## CHANGELOG - 2026-08-29 22:44 - 清理授权失败后的首屏缓存
...
- 首屏缓存提升了用户资料与会话列表的恢复速度，但远端明确拒绝访问时，缓存不能继续作为有效身份展示。
- 本次将 `403` 与既有认证失败路径统一处理，保证服务端授权结果优先于本地缓存。

### 改动概览

- `bootstrap-cache` 新增 `clearCachedSessions(userId)`。
- `App` 在 `401`、`403`、`502` 时清理 token、用户资料缓存和对应用户会话缓存，并置空当前用户状态。
- 新增 `403` 回归测试，验证三类缓存均被清理。

### 关键链路解析（含上下游）

- 上游：`UserBasicInfoGET` 负责确认当前 token 的有效性和用户归属。
- 当前改动：收到明确认证/授权失败后，`App` 读取缓存用户 ID，再删除该范围下的本地会话列表。
- 下游：`SessionSidebar` 不再从失效身份的缓存中恢复会话，页面回到匿名状态。

### 改动结果与业务影响

- 缓存只在远端身份仍可确认或网络暂时不可用时保留。
- 用户切换、权限撤销或失效 token 不会继续显示旧用户资料和会话元数据。

### 风险与待办

- `502` 仍沿用现有策略视为 token 失效；若网关未来将其用于临时上游故障，需要重新评估该语义。
- 建议后续补充 `401`、`502` 与网络失败下缓存保留策略的完整矩阵测试。

### 建议 Commit Message（git-cz）

- `fix(auth): clear cached data on forbidden response`
